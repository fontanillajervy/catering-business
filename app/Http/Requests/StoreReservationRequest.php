...
class StoreReservationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'contact_number' => ['bail', 'required', 'string', 'regex:/^\+63\d{10}$/', function ($attribute, $value, $fail) {
                $digits = preg_replace('/\D+/', '', $value ?? '');

                if (!$value || !preg_match('/^\+63\d{10}$/', $value)) {
                    $fail('Please enter a valid Philippine contact number in +63 format, for example +639123456789.');
                    return;
                }
    ...
    public function messages(): array
    {
        return [
            ...
            'contact_number.regex' => 'Please enter a valid Philippine contact number in +63 format, for example +639123456789.',
