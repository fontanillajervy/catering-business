...
const packageInput=document.getElementById('package_id'), guestInput=document.getElementById('guest_count'), budgetInput=document.getElementById('estimated_budget'); function updateBudget(){const option=packageInput.options[packageInput.selectedIndex], guests=Number(guestInput.value);if(option?.dataset.price&&guests) budgetInput.value=Number(option.dataset.price)*guests;} packageInput.addEventListener('change',updateBudget);guestInput.addEventListener('input',updateBudget);
</script>
@endsection
